...
//TODO when this is changed to BitFlags s_bitNameList needs to have "NONE" added as first entry

typedef BitfieldFlags<OBJECT_STATUS_COUNT, UnsignedInt>	ObjectStatusMaskType;
...
#if 0 // TODO restore when this is changed to BitFlags
...
// TODO remove when switched to BitFlags
inline /*static*/ void ObjectStatusMaskType::parseFromINI(INI* ini, void* instance, void *store, const void* /*userData*/)
{
	UnsignedInt bits = 0;
	INI::parseBitString32(ini, NULL, &bits, s_bitNameList);
	((ObjectStatusMaskType *)store)->m_bits = bits;
}

inline void ObjectStatusMaskType::xfer(Xfer* xfer)
{
	xfer->xferUnsignedInt(&m_bits);
}
